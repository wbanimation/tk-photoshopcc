...
#         _info_position = 0.32
    ...
    model_sheet_image_path = logo_image_path
    ...
    logger.info("(app) model_sheet_image_path %s. Exists: %s" % (
        model_sheet_image_path,
        os.path.exists(model_sheet_image_path)
    ))
...
#    engine.adobe.app.activeDocument.resizeImage(_width, _canvas_height, 200, engine.adobe.ResampleMethod.NONE)
    ...
    if project_type == '3D' or '2D':
        ...
        try :
            ...
            logger.error("app.open(model_sheet_image_path_file): %s" % e)
